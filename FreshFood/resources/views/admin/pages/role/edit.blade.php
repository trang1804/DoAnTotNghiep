...
<div class="card shadow mb-4 ">
    <div class="card-body">
        <div class="table-responsive">
            <form class="user" action="{{ route('cp-admin.user.role.update',[ 'id' => $roles->id ]) }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="form-group row">
                    <div class="col-sm-6 mb-3 mb-sm-0">
                        <label for="nameCategories">Chức vụ <span class="text-danger">(*)</span></label>
                        <input type="text" class="form-control form-control-user" id="name" value="{{ old('name', $roles->name ?? null) }}" name="name" id="namegories" placeholder="Tên danh mục ...">
                        @error('name')
                    ...
                    <div class="col-sm-6">
                        ...
                        <input type="text" class="form-control form-control-user " id="desc " value="{{ old('desc', $roles->desc ?? null) }}" name="desc" id="desc Categories" placeholder="Mô tả ...">
                        @error('desc')
                        <span class="text-danger">
                            {{$message}}
                        </span>
                            ...
                            <tbody>
                                ...
                                <tr class="checkboxRow">
                                    ...
                                    <td>
                                        <div class="form-check">
                                            <input 
                                             class="form-check-input checkbox_childen"
                                             type="checkbox" name="permission_id[]" 
                                             id="defaultCheck1 customCheckbox{{$permission->id}}" 
                                             value="{{$permission_Childen->id}}"
                                             
                                             {{ $roles->permissions->contains('id',$permission_Childen->id)? 'checked': '' }}
                                             >
                                            <label class="form-check-label" for="defaultCheck1">
